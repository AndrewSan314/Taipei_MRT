...
def _build_station_lookup_from_geojson(stations_geojson: dict) -> dict[str, dict]:
...
def _build_gis_route_context(network: object) -> GisRouteContext:
    fallback_bounds = (
    ...
    return GisRouteContext(
        ...
        station_lookup=_build_station_lookup_from_geojson(gis_payload["stations"]),
    )
...
def get_gis_route_context(network: object) -> GisRouteContext:
    ...
    context = _build_gis_route_context(network)
